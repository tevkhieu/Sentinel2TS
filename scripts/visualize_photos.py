...
def create_arg_parser():
    ...
    parser.add_argument(
        "--initial_time", type=int, default=0, help="Initial time from which the visualization begin"
    )
    return parser
...
def main():
    ...
    data = np.load(args.data_path)[args.initial_time:args.initial_time + args.time_span, [2, 1, 0], :, :].transpose(0, 2, 3, 1)
    ...
    slider_ax = plt.axes([0.25, 0.1, 0.65, 0.03], facecolor='lightgoldenrodyellow')  # Define the slider's position and size
    slider = Slider(slider_ax, 'Index', args.initial_time, args.initial_time + args.time_span - 1, valinit=args.initial_time, valstep=1)  # Define the slider itself
