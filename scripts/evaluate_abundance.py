...
def create_parser():
    ...
    parser.add_argument(
        "--good_permutation", type=int, nargs="+", help="Good permutation"
    )
...
def main():
    ...
    _, _, nb_endmember = abundance_map.shape
    predicted_abundance_map = np.load(args.predicted_abundance_path).clip(min=0)
    ...
    plt.imshow(
        np.sqrt(
            np.mean(
                (
                    abundance_map
                    - predicted_abundance_map[:, :, good_permutation]
                    / (np.sum(predicted_abundance_map, axis=2, keepdims=True) + 1e-6)
                )
                ** 2,
                axis=2,
            )
    ...
    fig, ax = plt.subplots(2, nb_endmember, figsize=(20, 10))
    for i in range(nb_endmember):
        ax[0, i].imshow(abundance_map[:, :, i], vmin=0, vmax=1)
        im = ax[1, i].imshow(
            predicted_abundance_map[:, :, good_permutation[i]]
            / (np.sum(predicted_abundance_map, axis=2) + 1e-6),
            vmin=0,
            vmax=1,
        )
        ax[1, i].set_title(
            f"Accuracy: {accuracy_list[i]:.2f}, RMSE: {rmse_list[i]:.2f}"
        )
        ax[0, i].axis("off")
        ax[1, i].axis("off")
...
def compute_abundance_rmse(
    ...
    predicted_abundance_map: np.ndarray,
    ...
    if good_permutation is None:
        predicted_abundance_map = predicted_abundance_map / (
            np.sum(predicted_abundance_map, axis=2, keepdims=True) + 1e-6
        )
        rmse = 1e9
        rmse_list = [1e9] * 4
        permutation_endmember = list(permutations(range(nb_endmember)))
        good_permutation = range(nb_endmember)
        for permutation in permutation_endmember:
            rmse_permutation = np.sqrt(
                np.mean(
                    (abundance_map - predicted_abundance_map[:, :, permutation]) ** 2
                )
            )
            if rmse_permutation < rmse:
                ...
                rmse_list = np.sqrt(
                    np.mean(
                        (abundance_map - predicted_abundance_map[:, :, permutation])
                        ** 2,
                        axis=(0, 1),
                    )
                )
...
def compute_accuracy(abundance_map, predicted_abundance_map, good_permutation=None):
    if good_permutation is None:
        ...
        for permutation in permutation_endmember:
            ...
            if accuracy_permutation > accuracy:
                ...
                accuracy_list = np.mean(
                    binary_abundance_map_flat == permuted_predicted, axis=0
                )
    else:
        binary_abundance_map = abundance_map >= 0.5
        binary_predicted_abundance_map = predicted_abundance_map >= 0.5
        ...
        accuracy = np.mean(
            np.all(
                binary_abundance_map
                == binary_predicted_abundance_map[:, :, good_permutation],
                axis=2,
            )
        )
        accuracy_list = np.mean(
